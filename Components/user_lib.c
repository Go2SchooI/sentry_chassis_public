...
uint8_t loop_int_constrain(uint8_t Input, int8_t minValue, int8_t maxValue)
{
    ...
    if (Input > maxValue)
    {
        uint8_t len = maxValue - minValue + 1;
        while (Input > maxValue)
        {
            Input -= len;
        }
...
{
    static float output;
    static float map_gain;
    map_gain = (map_max - map_min) / (intput_max - intput_min);
    output = intput * map_gain;
    if (intput < intput_min || intput > intput_max)
    {
        output = 0;
    }
    if (output > map_max)
